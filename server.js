...
// TEMPORARY — pick a real outreach_emails token + email the claim URL to admin for mobile UX testing.
// TODO: REMOVE after firing.
app.post('/api/admin/_diag/email-test-claim-link', async (req, res) => {
  try {
    // Find a recent valid token whose recipient still exists and isn't claimed yet
    const r = await pool.query(`
      SELECT oe.id, oe.recipient_id, oe.recipient_email, oe.unsubscribe_token,
             sc.full_name, sc.first_name, sc.last_name, sc.firm_name, sc.city, sc.province, sc.designation
      FROM outreach_emails oe
      ...
      WHERE oe.unsubscribe_token IS NOT NULL
        ...
        AND oe.sent_at >= '2026-04-08 00:00:00'
      ORDER BY oe.id DESC
      LIMIT 1
    `);
    if (r.rows.length === 0) {
      return res.status(404).json({ error: 'No suitable token found' });
    }
    const row = r.rows[0];
    const claimUrl = `https://canadaaccountants.app/claim-profile?ref=${row.unsubscribe_token}`;
    const apiCUrl = `https://canadaaccountants-backend-production-1d8f.up.railway.app/api/c/${row.recipient_id}`;

    const adminEmail = process.env.ADMIN_EMAIL || 'arthur@negotiateandwin.com';
    await sendEmail({
      to: adminEmail,
      subject: '🧪 Mobile UX test — claim page link',
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;">
          <h2 style="color:#1e3a8a;">Mobile UX Test — Claim Page</h2>
          <p>Tap the button below on your phone to experience the cold-recipient claim flow exactly as a real CPA would after clicking an outreach email.</p>
          <p><strong>Test recipient:</strong> ${row.full_name || row.first_name + ' ' + row.last_name} (${row.designation || 'CPA'}, ${row.city || ''} ${row.province || ''}${row.firm_name ? ', ' + row.firm_name : ''})</p>
          <table cellpadding="0" cellspacing="0" style="margin:24px auto;">
            <tr><td style="background:linear-gradient(135deg,#2563eb 0%,#1e3a8a 100%);border-radius:6px;padding:16px 36px;">
              <a href="${claimUrl}" style="color:#ffffff;text-decoration:none;font-size:18px;font-weight:600;display:inline-block;">📱 Open Claim Page (real URL)</a>
            </td></tr>
          </table>
          <p style="color:#666;font-size:13px;">Direct URL (also via the redirect endpoint /api/c/${row.recipient_id}):</p>
          <p style="font-family:monospace;font-size:11px;background:#f8fafc;padding:12px;word-break:break-all;border-left:3px solid #2563eb;">${claimUrl}</p>
          <p style="color:#666;font-size:13px;margin-top:24px;"><strong>What to test:</strong></p>
          <ol style="color:#333;font-size:14px;line-height:1.7;">
            <li>How does the page render on your phone? Anything obviously broken/hidden?</li>
            <li>Does the AI bio content make sense for this recipient? Wrong details would explain bounces.</li>
            <li>Is the form intimidating? How many fields, what does it ask?</li>
            <li>Does the CTA copy give you a clear reason to fill it out as a cold recipient?</li>
            <li><strong>DO NOT submit the form</strong> — that would mark this real CPA as claimed and trigger a magic link to them. Just observe and close.</li>
          </ol>
          <p style="color:#666;font-size:12px;margin-top:24px;">Token: <code>${row.unsubscribe_token}</code> | Recipient ID: ${row.recipient_id}</p>
        </div>
      `,
    });
    ...
    res.json({
      success: true,
      message: `Test claim link emailed to ${adminEmail}`,
      preview: {
        recipient: row.full_name,
        location: `${row.city}, ${row.province}`,
        firm: row.firm_name || '(none)',
        url: claimUrl,
      },
    });
  } catch (err) {
...
// 9:30 AM — Resend check (catches the post-9-AM send window's first 25 min, including
// any bounce/complaint webhook telemetry from Resend that lands shortly after sends)
cron.schedule('30 9 * * 2-4', () => runPipelineMonitor('9:30 AM Resend check').catch(e => console.error('[Monitor]', e.message)), { timezone: 'America/Toronto' });
...
console.log('[Monitor] Pipeline monitor scheduled: 9:05/9:30/10:05/14:05 Tue-Thu, 9:05/14:05 Fri');
