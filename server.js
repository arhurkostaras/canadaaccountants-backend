...
// TEMPORARY DIAGNOSTIC — full cold-cohort funnel for today + cumulative since 2026-04-08
// (the link fix). Splits clicks human/bot, shows where the funnel breaks.
app.get('/api/admin/_diag/funnel-deepdive', async (req, res) => {
  try {
    // Today's cold cohort funnel
    const todayQ = await pool.query(`
      SELECT
        ...
        COUNT(*) FILTER (WHERE clicked_at IS NOT NULL AND COALESCE(is_bot_click, false) = false) AS clicks_human,
        COUNT(*) FILTER (WHERE clicked_at IS NOT NULL AND is_bot_click = true) AS clicks_bot,
        COUNT(*) FILTER (WHERE real_visit_at IS NOT NULL) AS real_visits,
        COUNT(*) FILTER (WHERE status = 'converted') AS converted
      FROM outreach_emails
      WHERE campaign_id IS NOT NULL
        AND sent_at >= '2026-04-09 00:00:00'
    `);
    // Cumulative since link fix (2026-04-08)
    const sinceFixQ = await pool.query(`
      SELECT
        COUNT(*) FILTER (WHERE status IN ('sent','delivered','opened','clicked','bounced','complained','converted')) AS sent,
        COUNT(*) FILTER (WHERE clicked_at IS NOT NULL) AS clicks_total,
        COUNT(*) FILTER (WHERE clicked_at IS NOT NULL AND COALESCE(is_bot_click, false) = false) AS clicks_human,
    ...
    `);
    // A/B test page loads + completions today
    const abTodayQ = await pool.query(`
      SELECT
        COUNT(*) AS loads,
        COUNT(form_completed_at) AS completions
      FROM ab_test_results
      WHERE page_load_at >= '2026-04-09 00:00:00'
    `);
    // A/B cumulative since link fix
    const abSinceFixQ = await pool.query(`
      SELECT
        COUNT(*) AS loads,
        COUNT(form_completed_at) AS completions
      FROM ab_test_results
    ...
    const t = todayQ.rows[0];
    const f = sinceFixQ.rows[0];
    const ab_today = abTodayQ.rows[0];
    const ab_fix = abSinceFixQ.rows[0];

    const fmtPct = (n, d) => d > 0 ? (n / d * 100).toFixed(2) + '%' : '0%';

    res.json({
      success: true,
      cold_cohort_today: {
        sent: parseInt(t.sent, 10),
        opened: parseInt(t.opened, 10),
        clicks_total: parseInt(t.clicks_total, 10),
        clicks_human: parseInt(t.clicks_human, 10),
        clicks_bot: parseInt(t.clicks_bot, 10),
        real_visits: parseInt(t.real_visits, 10),
        converted: parseInt(t.converted, 10),
        funnel: {
          sent_to_open: fmtPct(t.opened, t.sent),
          sent_to_human_click: fmtPct(t.clicks_human, t.sent),
          human_click_to_real_visit: fmtPct(t.real_visits, t.clicks_human),
          real_visit_to_conversion: fmtPct(t.converted, t.real_visits),
          human_click_to_conversion: fmtPct(t.converted, t.clicks_human),
        },
      },
      cold_cohort_since_link_fix: {
        sent: parseInt(f.sent, 10),
        clicks_total: parseInt(f.clicks_total, 10),
        clicks_human: parseInt(f.clicks_human, 10),
        real_visits: parseInt(f.real_visits, 10),
        converted: parseInt(f.converted, 10),
        funnel: {
          sent_to_human_click: fmtPct(f.clicks_human, f.sent),
          human_click_to_real_visit: fmtPct(f.real_visits, f.clicks_human),
          human_click_to_conversion: fmtPct(f.converted, f.clicks_human),
        },
      },
      ab_test_today: {
        loads: parseInt(ab_today.loads, 10),
        completions: parseInt(ab_today.completions, 10),
        completion_rate: fmtPct(ab_today.completions, ab_today.loads),
      },
      ab_test_since_link_fix: {
        loads: parseInt(ab_fix.loads, 10),
        completions: parseInt(ab_fix.completions, 10),
        completion_rate: fmtPct(ab_fix.completions, ab_fix.loads),
      },
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
